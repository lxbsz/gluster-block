...
block_create_common(blockCreate *blk, char *control, char *volServer,
                    char *prio_path, size_t tcmur_timeout)
{
  char *tmp = NULL;
  char *backstore = NULL;
  char *tcmur_cmd_timeout = NULL;
  ...
  LOG("mgmt", GB_LOG_INFO,
      ...
      "tcmur_cmd_timeout=%lu", blk->volume, volServer?volServer:blk->ipaddr,
      blk->block_name, blk->block_hosts, blk->gbid, blk->auth_mode,
      blk->auth_mode?blk->passwd:"", blk->size, control, tcmur_timeout);
  ...
  if (gbTcmurCmdTimeoutDependenciesVersionCheck()) {
    if (GB_ASPRINTF(&tcmur_cmd_timeout, ";tcmur_cmd_time_out=%lu",
                    tcmur_timeout?tcmur_timeout:GB_TCMUR_CMD_TIMEOUT) == -1) {
      goto out;
    }
  }
  ...
  if (GB_ASPRINTF(&backstore, "%s %s name=%s size=%zu cfgstring=%s@%s%s/%s%s%s wwn=%s",
                  ...
                  blk->gbid, tcmur_cmd_timeout ? tcmur_cmd_timeout : "",
...
block_create_v2_1_svc_st(blockCreate2 *blk, struct svc_req *rqstp)
{
  ...
  if (len > 0 && xdata_val && GB_XDATA_IS_MAGIC(xdata_val->magic)) {
    ...
    case 4:
      tcmur_timeout = gbCreate->tcmur_timeout;
    case 3:
...
block_create_cli_1_svc_st(blockCreateCli *blk, struct svc_req *rqstp)
{
  ...
  LOG("mgmt", GB_LOG_INFO,
      ...
      "authmode=%d size=%lu rbsize=%d blksize=%d tcmur_cmd_timeout=%d",
  ...
  if (blk->tcmur_timeout) { // Create V4
    unsigned int len;
    ...
    gbCreate->tcmur_timeout = blk->tcmur_timeout;
