                               ...
                               bool rollback)
{
  ...
  if (blk->json_resp) {
    ...
    if (!errCode && mobj->auth_mode) {
      json_object_object_add(json_obj, "USERID",
...
block_gmodify_cli_1_svc_st(blockGModifyCli *blk, struct svc_req *rqstp)
{
  ...
  strcpy(mobj.userid, blk->userid);
  strcpy(mobj.passwd, blk->passwd);
...
block_modify_cli_1_svc_st(blockModifyCli *blk, struct svc_req *rqstp)
{
  ...
  if (!glfs_access(glfs, GB_VOLAUTH, F_OK)) {
    ...
    strcpy(mobj.userid, info->userid);
...
block_create_cli_1_svc_st(blockCreateCli *blk, struct svc_req *rqstp)
{
  ...
  if (info->auth_mode) {
    strcpy(cobj.userid, info->userid);
                           ...
                           struct blockResponse *reply)
{
  ...
  if (blk->json_resp) {
    ...
    json_object_object_add(json_obj, "USERID", GB_JSON_OBJ_TO_STR(info->userid));
